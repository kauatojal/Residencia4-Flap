...
// Mapeamento local dos nomes das colunas (strings) para os IDs da Lista (numéricos)
// ESTES IDs DEVEM CORRESPONDER AOS IDs REAIS DA TABELA 'lista' NO BANCO DE DADOS!
const COLUNA_PARA_LISTA_ID = {
    "INICIAR": 1, 
    "FAZENDO": 2,
    "Refação": 3,
    "Revisão": 4,
...
export default function KanbanBoard() {
    ...
    // --- FUNÇÃO DE MOVIMENTAÇÃO DE TAREFAS (CORRIGIDA PARA QUERY PARAMS) ---
    const onDragEnd = async (result) => {
        const { source, destination } = result;
        
        if (!destination || (source.droppableId === destination.droppableId && source.index === destination.index)) return;

        // 1. Atualização do Estado Local (Otimista)
        ...
        if (source.droppableId === destination.droppableId) {
            ...
            newColumns[source.droppableId] = startCol;
        } else {
            const finishCol = Array.from(columns[destination.droppableId]);
            finishCol.splice(destination.index, 0, movedCard);
            newColumns[source.droppableId] = startCol;
            newColumns[destination.droppableId] = finishCol;
        }

        setColumns(newColumns); // Aplica a mudança visual imediatamente

        // 2. Chamada à API para Persistir a Mudança
        if (source.droppableId !== destination.droppableId) {
            const tarefaId = result.draggableId;
            const novaListaNome = destination.droppableId;
            
            const novaListaId = COLUNA_PARA_LISTA_ID[novaListaNome];
            
            // NOTE: O ID do Quadro é fixo para este teste (Quadro Principal ID 1)
            const quadroId = 1; 

            if (novaListaId) {
                try {
                    // Chamada à API: Envia ambos os IDs como Query Parameters (Corrigido)
                    const url = `/tarefa/${tarefaId}/mover-lista?novaListaId=${novaListaId}&quadroId=${quadroId}`;
                    
                    await api.put(url, {}); // PUT sem corpo, apenas a URL com parâmetros

                    console.log(`Tarefa ${tarefaId} movida com sucesso para Lista ID: ${novaListaId} no Quadro ID: ${quadroId}`);
                    
                } catch (error) {
                    console.error("Erro ao mover tarefa via API:", error.response || error);
                    alert("Movimentação falhou no servidor. Permissão negada ou ID inválido.");
                    
                    // Reverter o estado local se a API falhar (Melhor Prática)
                    setColumns(columns); 
                }
            } else {
                 console.error(`ERRO: Mapeamento de lista não encontrado para coluna: ${novaListaNome}`);
                 setColumns(columns);
            }
        }
    };
    ...
    // --- FUNÇÃO CORRIGIDA DE CRIAÇÃO DE TAREFA (Mapeamento) ---
    const handleAddTask = (apiTaskData) => {
        
        const mapApiTaskToCard = (apiTask) => {
            if (!apiTask || !apiTask.id) {
                console.error("Dados da API inválidos ou incompletos.");
                return null;
            }

            const numParticipantes = apiTask.responsaveisIds ? apiTask.responsaveisIds.length : 0;
            const tagNome = apiTask.flagsTarefaIds?.[0]?.nome || 'Normal';
            const prazoFormatado = apiTask.prazoFim 
                ? new Date(apiTask.prazoFim).toLocaleDateString('pt-BR') 
                : 'Sem Prazo';

            return {
                id: String(apiTask.id),
                name: apiTask.titulo || 'Sem Título', 
                descricao: apiTask.descricao || '',
                sector: apiTask.setorId?.nome || 'Design', 
                client: apiTask.clienteId?.nome || 'Cliente Não Associado', 
                ...
                comentarios: [], 
                anexos: [], 
                membros: apiTask.responsaveisIds.map(r => ({ id: r.id })) 
            };
        };


        const newTask = mapApiTaskToCard(apiTaskData); 
    ...
    const handleDeleteCard = () => {
        ...
        // Lógica de exclusão no backend DEVE ser chamada aqui
    ...
    // --- FUNÇÃO DE RENDERIZAÇÃO DO KANBAN ---
