          ...
          <tbody>
            {usuarios.map((user) => (
              <tr key={user.id}>
                ...
                {/* ✅ COLUNA DE CARGOS */}
                <td data-label="Cargos" className="cargos-cell">
                  {user.cargosIds && user.cargosIds.length > 0 ? (
                    <div className="cargos-wrapper">
                      {user.cargosIds.map((cargo) => (
                        <span key={cargo.id} className="cargo-badge">
                          {cargo.nome}
                        </span>
                      ))}
                    </div>
                  ) : (
                    <span className="sem-cargo">Sem cargo</span>
                  )}
                </td>
